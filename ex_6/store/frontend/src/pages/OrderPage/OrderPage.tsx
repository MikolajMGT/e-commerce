...
const CreditCardPopupModal: FC = () => (
	...
	<Popup closeOnDocumentClick={false} trigger={
		<IconButton style={{color: '#DEE2E6'}}>
			...
			<Typography variant="button">Add credit card</Typography>
		</IconButton>}
	       nested modal>
		{(close: any) => (
		...
		return addresses?.map((address: AddressDb) => {
			return (
				<Col id={uuidv4()} style={{
					cursor: 'pointer',
					borderRadius: '4px',
					background: address === selectedAddress ? theme.palette.secondary.dark : 'inherit'
				}
		...
		return creditCards?.map((card: CreditCardDb) => {
			return (
				<Col id={uuidv4()} style={{
					cursor: 'pointer',
					borderRadius: '4px',
					background: card === selectedCard ? theme.palette.secondary.dark : 'inherit'
				}
			...
			return (
				<Row>
					<div className="order-summary-item mt-2 mb-2" id={uuidv4()}>
						<ImageStyled height={'8vh'} width={'12vh'} image={product.image}/>
		...
		<Page>
			<OrderPageStyled key={uuidv4()}>
				...
				<Container>
						...
						<Col>
							<div className="entries mt-5">
								<div className="text-center mb-3">
									...
									<Typography variant="button">Selected Products</Typography>
								</div>
								{prepareSummary()}
							</div>
						</Col>
					...
					<Row>
						<div className="entries mt-4" style={{maxWidth: '74vw'}}>
							<Row>
								<Col className='col-10'>
									<DialogContentText style={{color: '#FCFDFE'}}>Discount Code:</DialogContentText>
								...
								<Col className='col-2'>
									...
									<Button style={{height: '40px', borderRadius: '20px'}} variant="contained" color="secondary" onClick={() => onSubmit()}>
										Submit Order
									</Button>
								</Col>
							</Row>
