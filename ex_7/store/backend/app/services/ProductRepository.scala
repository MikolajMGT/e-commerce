...
class ProductRepository @Inject()(dbConfigProvider: DatabaseConfigProvider, val stockRepository: StockRepository, val categoryRepository: CategoryRepository, val subcategoryRepository: SubcategoryRepository)(implicit ec: ExecutionContext) {
  ...
  def create(stockId: Long, categoryId: Long, subcategoryId: Long, name: String, imageUrl: String, description: String, createdAt: Timestamp = Timestamp.from(Instant.now()), updatedAt: Timestamp = Timestamp.from(Instant.now())): Future[Product] = db.run {
    (product.map(p => (p.stockId, p.categoryId, p.subcategoryId, p.name, p.imageUrl, p.description, p.createdAt, p.updatedAt))
      returning product.map(_.id)
      into { case ((stockId, categoryId, subcategoryId, name, imageUrl, description, createdAt, updatedAt), id) => Product(id, stockId, categoryId, subcategoryId, name, imageUrl, description, createdAt, updatedAt) }
      ) += (stockId, categoryId, subcategoryId, name, imageUrl, description, createdAt, updatedAt)
  }
