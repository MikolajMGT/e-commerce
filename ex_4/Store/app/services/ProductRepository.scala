...
class ProductRepository @Inject()(dbConfigProvider: DatabaseConfigProvider, val stockRepository: StockRepository, val categoryRepository: CategoryRepository, val subcategoryRepository: SubcategoryRepository)(implicit ec: ExecutionContext) {
  ...
  class ProductTable(tag: Tag) extends Table[Product](tag, "product") {
    ...
    def * = (id, stockId, categoryId, subcategoryId, name, description, createdAt, updatedAt) <> ((Product.apply _).tupled, Product.unapply)
  }
  ...
  def create(stockId: Long, categoryId: Long, subcategoryId: Long, name: String, description: String, createdAt: Timestamp = Timestamp.from(Instant.now()), updatedAt: Timestamp = Timestamp.from(Instant.now())): Future[Product] = db.run {
    (product.map(p => (p.stockId, p.categoryId, p.subcategoryId, p.name, p.description, p.createdAt, p.updatedAt))
      returning product.map(_.id)
      into { case ((stockId, categoryId, subcategoryId, name, description, createdAt, updatedAt), id) => Product(id, stockId, categoryId, subcategoryId, name, description, createdAt, updatedAt) }
      ) += (stockId, categoryId, subcategoryId, name, description, createdAt, updatedAt)
  }
