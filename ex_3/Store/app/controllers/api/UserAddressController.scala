...
class UserAddressController @Inject()(val userAddressRepo: UserAddressRepository, cc: ControllerComponents)(implicit exec: ExecutionContext) extends AbstractController(cc) {
  ...
  def getUserAddressById(id: String): Action[AnyContent] = Action.async {
    ...
    val userAddress = userAddressRepo.getByIdOption(id.toLong)
    userAddress.map {
  ...
  def listUserAddressesByUserId(userId: String): Action[AnyContent] = Action.async {
    logger.info(s"listUserAddressesByUserId($userId)")

    val userAddresss = userAddressRepo.listByUserId(userId.toLong)
    userAddresss.map { userAddresses =>
      Ok(Json.toJson(userAddresses))
    }
  }
  ...
  def deleteUserAddress(id: String): Action[AnyContent] = Action.async {
    ...
    userAddressRepo.delete(id.toLong).map { res =>
