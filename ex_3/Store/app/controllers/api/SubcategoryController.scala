...
class SubcategoryController @Inject()(val subcategoryRepo: SubcategoryRepository, cc: ControllerComponents)(implicit exec: ExecutionContext) extends AbstractController(cc) {
  ...
  def getSubcategoryById(id: String): Action[AnyContent] = Action.async {
    logger.info(s"getSubcategoryByName($id)")

    val subcategory = subcategoryRepo.getByIdOption(id.toLong)
    subcategory.map {
  ...
  def listSubcategoriesByCategoryId(categoryId: String): Action[AnyContent] = Action.async {
    logger.info(s"listSubcategoriesByCategoryId($categoryId)")

    val subcategories = subcategoryRepo.listByCategoryId(categoryId.toLong)
    subcategories.map { subcategories =>
      Ok(Json.toJson(subcategories))
    }
  }
  ...
  def deleteSubcategory(id: String): Action[AnyContent] = Action.async {
    ...
    subcategoryRepo.delete(id.toLong).map { res =>
