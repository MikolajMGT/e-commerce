...
class OrderController @Inject()(val orderRepo: OrderRepository, cc: ControllerComponents)(implicit exec: ExecutionContext) extends AbstractController(cc) {
  ...
  def getOrderById(id: String): Action[AnyContent] = Action.async {
    ...
    val order = orderRepo.getByIdOption(id.toLong)
    order.map {
  ...
  def listOrdersByUserId(userId: String): Action[AnyContent] = Action.async {
    logger.info(s"listOrdersByUserId($userId)")

    val orders = orderRepo.listByUserId(userId.toLong)
    orders.map { orders =>
      Ok(Json.toJson(orders))
    }
  }

  def listOrdersByPaymentId(paymentId: String): Action[AnyContent] = Action.async {
    logger.info(s"listOrdersByPaymentId($paymentId)")

    val orders = orderRepo.listByPaymentId(paymentId.toLong)
    orders.map { orders =>
      Ok(Json.toJson(orders))
    }
  }

  def listOrdersByVoucherId(voucherId: String): Action[AnyContent] = Action.async {
    logger.info(s"listOrdersByVoucherId($voucherId)")

    val orders = orderRepo.listByVoucherId(voucherId.toLong)
    orders.map { orders =>
      Ok(Json.toJson(orders))
    }
  }
  ...
  def deleteOrder(id: String): Action[AnyContent] = Action.async {
    ...
    orderRepo.delete(id.toLong).map { res =>
